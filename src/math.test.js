...
describe('função fatorial', () => {
  test('valida entrada', () => {
    ...
    expect(() => fatorial(10)).not.toThrow();  // Corrigido para verificar se o número é válido
  ...
  test('calcula fatorial de 0 para ser igual a 1', () => {
    expect(fatorial(0)).toBe(1);  // Corrigido de 'factorial' para 'fatorial'
  });
});

// Teste para a função Fibonacci
describe('função fibonacci', () => {
  test('valida entrada', () => {
    expect(fibonacci).toBeDefined();  // Corrigido nome da função para 'fibonacci'
    expect(fibonacci.length).toBe(1);  // Verifica se a função possui 1 argumento
    expect(() => fibonacci("Texto")).toThrow(TypeError);
    expect(() => fibonacci(10)).not.toThrow();
    expect(() => fibonacci(0)).toThrow("O número deve ser maior que zero");
  });
