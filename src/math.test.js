...
describe("função fibonacci", () => {
  test("valida entrada", () => {
    ...
    expect(() => fibonacci("Texto")).toThrow(
      "O argumento deve ser um número válido"
    );
    expect(() => fibonacci("10")).not.toThrow(TypeError);
    expect(() => fibonacci(10)).not.toThrow(TypeError);

    // Garantir que não existe fibonacci de número menor ou ingual a zero
    expect(() => fibonacci(-10)).toThrow(Error);
    expect(() => fibonacci(-10)).toThrow(
      "Não existe fibonacci de número menor ou igual a zero"
    );
...
describe("função ehPrimo", () => {
  test("valida entrada", () => {
    ...
    expect(() => ehPrimo(1)).toThrow(
      "Não número primo de número menor ou igual a 1"
    );
    expect(() => ehPrimo(0)).toThrow(Error);
    expect(() => ehPrimo(0)).toThrow(
      "Não número primo de número menor ou igual a 1"
    );
    expect(() => ehPrimo(-2)).toThrow(Error);
    expect(() => ehPrimo(-2)).toThrow(
      "Não número primo de número menor ou igual a 1"
    );
  });
